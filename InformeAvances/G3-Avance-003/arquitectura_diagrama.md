...
flowchart TD
    %% Estilos compactos
    classDef data fill:#f5f5f5,stroke:#666,stroke-width:1.5px;
    classDef logic fill:#dae8fc,stroke:#6c8ebf,stroke-width:1.5px;
    classDef ui fill:#d5e8d4,stroke:#82b366,stroke-width:1px;

    %% Capas Verticales
    subgraph Capa_Datos [Capa de Datos: GAMA Simulator]
        CSV["Fuentes .csv (Métricas, Nodos,<br/>Topología, Eventos)"]:::data
    end

    subgraph Capa_Logica [Middleware: Vue 3 + Vite]
        Papa["PapaParse (Async Parser)"]:::logic
        State["Vue Reactive State (Local)"]:::logic
    end

    subgraph Capa_UI [Interfaz de Monitoreo]
        Cytoscape["1. Mapa de Red (Cytoscape)"]:::ui
        ECharts["2. Panel de Métricas (ECharts)"]:::ui
        Table["3. Tabla de Incidentes (BDI)"]:::ui
    end

    %% Flujo Lineal (Ahorra ancho)
    CSV -->|Polling 2s| Papa
    Papa -->|Objects JSON| State
    State -->|Reactive Binding| Cytoscape
    State -->|Reactive Binding| ECharts
    State -->|Reactive Binding| Table
```
